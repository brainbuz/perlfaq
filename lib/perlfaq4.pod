...
Unlike the others, Perl allows you to embed a variable naked in a quoted
string C<"variable $variable">, when there isn't whitespace around the
variable name you can add braces like C<"foo ${foo}bar">.

An array can also be embedded directly in a string, and will be expanded
by default with spaces between the elements. The default expansion can be
changed by assigning a different string to the special variable B<$">,
such as C<$" = ', ';>.
...
When you want a list to be expanded per B<$">, use C<@{[ ... ]}>.
    ...
    $" = ':';
    say "Set \$\" to \':\' and sort the Array @{[ sort @array ]}";
